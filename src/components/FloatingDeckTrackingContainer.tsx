...
const FloatingDeckTrackingContainer: React.FC = () => {
  ...
  // Read FloatingDeck_tracking.json from GAME_PATH and subscribe to updates
  useEffect(() => {
    ...
    const loadFloatingDeck = async () => {
      try {
        ...
        const floatingDeckPath = `${gamePath}/FloatingDeck_tracking.json`;
        const content = await window.electron.readFile(floatingDeckPath);
        const parsed: FloatingDeckData = JSON.parse(content || '{"cards":{}}');
        setDeckData(parsed);
      } catch (error) {
        console.error('Failed to load FloatingDeck_tracking.json:', error);
      }
    };
  ...
  return (
    <div className="floating-match-container" style={{ marginBottom: '10px' }}>
      <div className="floating-match-title">追踪卡牌</div>
...
export default FloatingDeckTrackingContainer;
